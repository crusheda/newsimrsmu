...
namespace App\Http\Controllers\v4\Administrasi\Berkas;
...
class LaporanBulananController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pages.v4.administrasi.berkas.laporan.index')->with('list', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        //     'bln'   => 'required|integer|min:1|max:12',
        //     'thn'   => 'required|integer|digits:4',
        //     'judul' => 'required|string|max:255',
        //     'ket'   => 'nullable|string|max:1000',

        //     // maksimal 5MB = 5120 KB
        //     'file'  => 'required|file|mimes:pdf,doc,docx|max:5120',
        // ], [
        //     'bln.required'   => 'Bulan wajib dipilih',
        //     'bln.integer'    => 'Format bulan tidak valid',
        //     'bln.min'        => 'Bulan tidak valid',
        //     'bln.max'        => 'Bulan tidak valid',

        //     'thn.required'   => 'Tahun wajib dipilih',
        //     'thn.integer'    => 'Format tahun tidak valid',
        //     'thn.digits'     => 'Tahun harus 4 digit',

        //     'judul.required' => 'Judul laporan wajib diisi',
        //     'judul.max'      => 'Judul maksimal 255 karakter',

        //     'file.required'  => 'Dokumen wajib diupload',
        //     'file.mimes'     => 'Dokumen harus berupa PDF atau Word (.doc/.docx)',
    ...
    public function formVerif($id)
    {
        ...
        if ($user->can('admin_laporan_bulanan_verifall')) {
            $res = 1;
            return response()->json($res, 200);
        } else {
            $cek = struktur_organisasi::where('id_user',$id)->first();
            if ($cek && !empty($cek->nama_user)) {
                $res = 1;
                return response()->json($res, 200);
            } else {
                $res = 2;
    ...
    function showVerif()
    {
        ...
        if ($user->can('admin_laporan_bulanan_verifall')) {
            return view('pages.v4.administrasi.berkas.laporan.verif');
        } else {
            if (!empty($cek->nama_user)) {
                return view('pages.v4.administrasi.berkas.laporan.verif');
            } else {
