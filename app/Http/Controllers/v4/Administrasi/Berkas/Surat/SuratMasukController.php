...
namespace App\Http\Controllers\v4\Administrasi\Berkas\Surat;
...
class SuratMasukController extends Controller
{
    ...
    {
        if (Auth::user()->can('surat_masuk') == true) {
            ...
            return view('pages.v4.administrasi.berkas.surat.suratmasuk.index')->with('list', $data); //
        } else {
    ...
    public function acAsal(Request $request)
    {
        ...
        $data = surat_masuk::where('asal', 'like', "%{$keyword}%")
            ->distinct()
            ->orderBy('asal')
            ->limit(10)
            ->pluck('asal');
    ...
    public function acTempat(Request $request)
    {
        ...
        $data = surat_masuk::where('tempat', 'like', "%{$keyword}%")
            ->distinct()
            ->orderBy('tempat')
            ->limit(10)
            ->pluck('tempat');
