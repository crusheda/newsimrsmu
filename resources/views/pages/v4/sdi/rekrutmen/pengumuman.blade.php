...
@section('content')
        ...
        <div class="page-header-breadcrumb mb-3">
            <div class="d-flex align-center justify-content-between flex-wrap">
                <h1 class="page-title fw-medium fs-18 mb-0 pe-none">
                    Daftar <b class="text-danger link-underline-danger text-decoration-underline">Pengumuman</b>
                </h1>
            ...
            <div class="col-sm-12">
                <div class="card custom-card">
                    <div class="card-header d-flex align-items-center justify-content-between py-3">
                        ...
                        <div class="btn-group">
                            <button type="button" class="btn btn-warning" onclick="refresh()" id="btn-refresh"><i class="ri-refresh-line"></i></button>
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambah"><i class="ri-add-box-line me-1"></i> Tambah Loker</button>
                            <button type="button" class="btn btn-info" onclick="window.location='{{ route('v4.sdi.rekrutmen.registrasi') }}'"><i class="ri-group-line me-1"></i> Lihat Peserta</button>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="alert alert-light shadow-sm mb-3">
                            <small>
                                <i class="fa-fw fas fa-caret-right nav-icon"></i> <b>Hapus Lowongan Kerja</b> digunakan <b class="text-danger">HANYA</b> apabila lowongan salah/dibatalkan <br>
                                <i class="fa-fw fas fa-caret-right nav-icon"></i> Lowongan akan tampil pada Website RS setelah input pada rentang <b class="text-orange">tanggal dibuka sampai ditutup</b> <br>
                            </small>
                        </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">
                        Tambah <b class="text-primary">Lowongan Pekerjaan</b>
                    </h6>
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            <div class="alert alert-light shadow-sm">
                                <small>
                                    <i class="fa-fw fas fa-caret-right nav-icon"></i> Tanda (<a class="text-danger">*</a>) berarti isian <b class="text-primary">Wajib</b> diisi<br>
                                    <i class="fa-fw fas fa-caret-right nav-icon"></i> Pastikan Tgl. <b class="text-info">Buka Lowongan</b> tidak lebih dari Tgl. <b class="text-danger">Tutup Lowongan</b><br>
                                    <i class="fa-fw fas fa-caret-right nav-icon"></i> Disarankan untuk menggunakan <b class="text-warning">Huruf Besar/Capital/Uppercase</b> saat pengisian
                                </small>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="form-group">
                                <label class="form-label">Maksimal Kuota Pendaftar (<b class="text-orange">Optional</b>)</label>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <label class="form-label">Keterangan (<b class="text-orange">Optional</b>)</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="prosesSimpan()" id="btn-simpan"><i class="fa-fw fas fa-save nav-icon me-1"></i> Submit</button>
                </div>
            </div>
        </div>
    </div>
    <div class="modal fade bd-example-modal-lg" id="ubah" role="dialog" aria-labelledby="confirmFormLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">
                        Ubah <b class="text-warning">Lowongan Pekerjaan</b> <span class="badge text-bg- p-1 ms-1">ID#<b id="show_id_edit"></b></span>
                    </h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            <div class="form-group">
                                <label class="form-label">Token (<b class="text-danger">Kredensial</b>)</label>
    ...
    <script>
        ...
        function refresh() {
            ...
            $.ajax(
                {
                    url: "/api/v4/sdi/rekrutmen/pengumuman/table",
                    type: 'GET',
                    ...
                    success: function(res) {
                        ...
                        var kepalaID = @json(Auth::user()->can('admin_kepegawaian_kepala'));
                        $("#tampil-tbody").empty();
                        $('#dttable').DataTable().clear().destroy();

                        res.show.forEach(item => {
                            content = "<tr id='data"+ item.id +"'>";
                            content += `<td><center><div class='dropend'><a href='javascript:void(0);' class='btn btn-primary-transparent btn-sm text-muted font-size-16 rounded' data-bs-auto-close='true' data-bs-toggle='dropdown' aria-haspopup="true"><i class="ti ti-dots"></i></a><div class='dropdown-menu'>`;
                                ...
                                if (item.status == 1) {
                                    if (moment(res.now).format('YYYY-MM-DD') >= moment(item.mulai).format('YYYY-MM-DD')) {
                                        if (moment(res.now).format('YYYY-MM-DD') <= moment(item.selesai).format('YYYY-MM-DD')) {
                                            ...
                                            content += `<a href='javascript:void(0);' class='dropdown-item disabled'><i class='fas fa-trash me-1'></i> Non Aktif</a>`;
                                        }
                                    } else {
                                        content += `<a href='javascript:void(0);' class='dropdown-item disabled'><i class='fas fa-edit me-1'></i> Ubah</a>`;
                                        content += `<a href='javascript:void(0);' class='dropdown-item disabled'><i class='fas fa-trash me-1'></i> Non Aktif</a>`;
                            ...
                            content += `<td style='white-space: normal !important;word-wrap: break-word;'>
                                            <div class='d-flex justify-content-start align-items-center'>
                                                <div class='d-flex flex-column'>
                                                    <h6 class='mb-0'>${item.nama} ${item.unit?'<b class="text-primary">('+item.unit+')</b>':''}</h6>
                                                    ...
                                                    <small class='text-truncate text-muted'>Batasan Umur : <b>${item.umur_min?item.umur_min:'x'} - ${item.umur_max?item.umur_max:'x'} Tahun</b></small>
                                                </div>
                                            </div>
                                        </td>`;
        ...
        function ubah(id) {
            $.ajax(
                {
                    url: `/api/v4/sdi/rekrutmen/pengumuman/${id}/show`,
                    type: 'GET',
                    dataType: 'json', // added data type
                    success: function(res) {
            ...
            if ($('.notnull_edit').val() == "") {
                ...
                if ($('#selesai_edit').val() < $('#mulai_edit').val()) {
                    ...
                    $.ajax({
                        ...
                        method: 'POST',
                        url: '/api/v4/sdi/rekrutmen/pengumuman/ubah',
                        contentType: false,
                        processData: false,
                        dataType: 'json',
                        data: save,
                        beforeSend: function() {
                            btn.prop('disabled', true);
                            btn.find("i").removeClass('fa-edit').addClass("fa-sync fa-spin");
                        },
                        success: function(res) {
                            if (res.code == 400) {
                                iziToast.error({
                        ...
                        complete: function() {
                            btn.find("i").removeClass("fa-sync fa-spin").addClass('fa-edit');
                            btn.prop('disabled', false);
                        },
                        error: function (res) {
                        }
            ...
            if ($('.notnull').val() == "") {
                ...
                if ($('#selesai').val() < $('#mulai').val()) {
                    ...
                    $.ajax({
                        ...
                        method: 'POST',
                        url: '/api/v4/sdi/rekrutmen/pengumuman/simpan',
                        contentType: false,
                        processData: false,
                        dataType: 'json',
                        data: save,
        ...
        function prosesNonAktif() {
            ...
            if (checkboxHapus == false) {
                ...
                $.ajax({
                    ...
                    url: "/api/v4/sdi/rekrutmen/pengumuman/nonaktif/"+id+"",
                    type: 'DELETE',
                    beforeSend: function() {
                    ...
                    error: function(xhr) {
                        iziToast.error({
                            title: 'Pesan Galat!',
                            message: xhr.responseJSON.message ?? 'Lowongan Pekerjaan gagal dihapus',
                            position: 'topRight'
                        });
                    },
                    complete: function() {
